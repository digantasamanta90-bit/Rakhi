    ...
    return new Promise((resolve) => {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        for (let j = 0; j < windowCount; j++) {
          ...
          windows += `<div style="width:4px;height:4px;background:${lit ? 'rgba(255,220,150,0.6)' : 'rgba(250,245,238,0.03)'};margin:3px auto;"></div>`;
        }
        buildingsHTML += `<div style="flex-shrink:0;width:${w}px;height:${h}px;background:var(--cinema-deep);border:1px solid rgba(250,245,238,0.03);align-self:flex-end;margin-right:2px;display:flex;flex-direction:column;justify-content:flex-start;padding-top:4px;">${windows}</div>`;
      }

      container.innerHTML = `
        <div class="journey-container">
          <!-- Sky gradient -->
          <div style="position:absolute;inset:0;background:linear-gradient(180deg, #1a1520 0%, #1f1822 40%, #2b1f24 70%, #0a0608 100%);"></div>

          <!-- Stars/lights far layer -->
          <div id="j-stars" style="position:absolute;top:0;left:0;width:200%;height:30%;">
            ${Array.from({length:20}).map(() =>
              `<div style="position:absolute;width:2px;height:2px;background:rgba(255,255,255,${0.1+Math.random()*0.3});border-radius:50%;top:${Math.random()*100}%;left:${Math.random()*100}%;"></div>`
            ).join('')}
          </div>
          ...
          <!-- Ground/road -->
          <div style="position:absolute;bottom:0;left:0;width:100%;height:25%;background:linear-gradient(180deg, #1a1216, #0a0608);"></div>

          <!-- Light streaks -->
          <div id="j-streaks" style="position:absolute;bottom:15%;left:0;width:200%;height:8%;display:flex;gap:30px;">
            ${Array.from({length:15}).map(() =>
              `<div style="flex-shrink:0;width:${40+Math.random()*80}px;height:2px;background:rgba(200,162,72,${0.1+Math.random()*0.2});border-radius:1px;align-self:center;"></div>`
            ).join('')}
          </div>

          <!-- Window frame -->
          <div class="journey-window-frame"></div>
          ...
          <div style="position:absolute;top:0;right:0;width:40%;height:100%;background:linear-gradient(135deg, rgba(255,255,255,0.02) 0%, transparent 60%);pointer-events:none;z-index:11;"></div>
        </div>
      `;
